...
class news:
    ...
    def baiduRss(self, keyword, page=0):
        ...
        keywordInfo = self.mp.findOne('park_news_keyword', keycriteria)
        try:
            keywordInfo['park_id'] = int(keywordInfo['park_id'])
        except:
            keywordInfo['park_id'] = str(0)
       # print rss
	...
	#print rss.entries
        for newsInfo in rss.entries:
            ...
            newsData = self.mp.findOne('park_news_primary', criteria)
	 #   print 'cccccccccccccccc'
            if not newsData:
	#	print 'bbbbbbbbbbbbbbbbbb'
                isInsert = self.insert(newsInfo, uniqueKey, keyword, int(keywordInfo['park_id']))
	#	print 'aaaa:'+isInsert
                if isInsert == False:
                    return False
            else:
    ...
    def insert(self, newsInfo, uniqueKey, keyword, park_id):
	...
	desc = newsInfo.summary;
        ...
        data = dict(unique_key=uniqueKey, title=newsInfo.title, park_name=keyword, description=MySQLdb.escape_string(sub_desc), url=newsInfo.link,
                    publish_time=publishTime, source=newsInfo.author, insert_time=time.strftime("%Y-%m-%d %H:%M:%S"))
        if park_id > 0:
            data['park_id'] = park_id
        self.mp.save('park_news_primary',data)

        newsInfo = self.content(newsInfo.link)

        info = dict(unique_key=uniqueKey, content=MySQLdb.escape_string(newsInfo))
        self.mp.save('park_news_detail', info)
