...
class ShanghaisixSpider(scrapy.Spider):
    name = 'shanghaisix'
    allowed_domains = ['www.sheitc.gov.cn']
    start_urls = ['http://www.sheitc.gov.cn/xxfw/index_1.htm']
    baseurl = "http://www.sheitc.gov.cn/xxfw/index_"
    num = 2

    def parse(self, response):
        # list = response.xpath("//p/a/@href").extract()
        r = re.findall(r"http.*xxfw.*?htm", response.body)
        for i in r:
            print i
